function output = flappybird(network)
...
while 1
    ...
    while 1
        ...
        loops = 0;
        curTime = toc(stageStartTime);
        if abs(curTime - oldTime) > 0.125
            oldTime = curTime;        
            if ~Flags.PreGame && Flags.BotStartedGame && ~gameover                
                if Tubes.ScreenX(todisp) - Bird.ScreenPos(1) + TUBE.WIDTH > 0
                    distanceToPipe = Tubes.ScreenX(todisp) - Bird.ScreenPos(1);
                    birdHeight = Bird.ScreenPos(2);
                    pipeOpeningHeight = 151 - (Tubes.VOffset(todisp));
                    if distanceToPipe <= 3
                        distanceTpPipe = distanceTpPipe;
                    end
                    FlyKeyStatus = network.evaluate(birdHeight, pipeOpeningHeight, distanceToPipe);
        ...
        while (curTime >= ((CurrentFrameNo) * GAME.FRAME_DURATION) && loops < GAME.MAX_FRAME_SKIP)            
          ...
          %% Cycling the Palette
           ...
           if collide
               ...
               birdHeight = Bird.ScreenPos(2);
               pipeOpeningHeight = 151 - (Tubes.VOffset(todisp));
               addition = 1/(1 + abs(birdHeight - pipeOpeningHeight)/10);
               output = curTime + addition;
           end
           CurrentFrameNo = CurrentFrameNo + 1;
           loops = loops + 1;
           frame_updated = true;
